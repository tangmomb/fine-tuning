...
.\.venv\Scripts\python.exe 02_evaluation_brut_models\run_openai.py
```
...
```powershell
python 02_evaluation_brut_models\assets\run_evaluation.py --phase generate --model models\Qwen3.5-4B --mode zero-shot --device cuda --batch-size 16 --environment scaleway --run-name h100-qwen4b-zs --save-prompts
```
...
```powershell
.\.venv\Scripts\python.exe 02_evaluation_brut_models\assets\run_evaluation.py --phase evaluate --predictions 02_evaluation_brut_models\runs\h100-qwen4b-zs\predictions.jsonl --run-name h100-qwen4b-zs-local-score
```
...
```powershell
.\.venv\Scripts\python.exe 02_evaluation_brut_models\run.py
```

Pour une exécution non interactive :

```powershell
.\.venv\Scripts\python.exe 02_evaluation_brut_models\run.py --environment scaleway --models models\Qwen3.5-4B --mode both
```
