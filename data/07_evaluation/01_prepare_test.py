...
def correction_map() -> dict[str, str]:
    path = ROOT / "data" / "05_checks" / "production" / "manual_corrections.jsonl"
    if not path.is_file():
        return {}
    return {
        row["id"]: row["question"]
...
def main() -> None:
    ...
    checks_path = ROOT / "data" / "05_checks" / "production" / "test_deterministic_checks.jsonl"
    judgments_path = ROOT / "data" / "05_checks" / "production" / "sol_judgments_test.jsonl"
