...
def correction_map(environment):
    ...
    path = ROOT / "01_data" / "05_quality_control" / folder / "02_manual_corrections" / "manual_corrections.jsonl"
    if not path.is_file():
        return {}
    return {
        row["id"]: row["question"]
...
def main():
    ...
    folder = {"pilot": "02_pilot", "production": "03_production"}[environment]
    translations_path = ROOT / "01_data" / "04_merge_translations" / folder / "train_spider.jsonl"
    judgments_path = ROOT / "01_data" / "05_quality_control" / folder / "09_judgments" / "sol_judgments.jsonl"
    output = ROOT / "01_data" / "06_training_dataset" / folder / "train.jsonl"
    manifest = ROOT / "01_data" / "06_training_dataset" / folder / "manifest.json"
    if environment == "production":
        ...
        for split in ("train_spider", "train_others", "dev"):
            translations = load(ROOT / "01_data" / "04_merge_translations" / folder / f"{split}.jsonl")
            checks = {row["id"]: row for row in load(ROOT / "01_data" / "05_quality_control" / folder / "01_deterministic_checks" / f"{split}_deterministic_checks.jsonl")}
